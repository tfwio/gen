...
namespace Generator.Parser
{
	...
	static public class SqlTemplateParser
	{
		...
		static readonly string[] checkAccess = Gen.Strings.Schema_Access_Fields.Split(',');
...
/// <summary>
		...
		static public string ParseSqlTemplate(string input, DataSet ds)
		{
          ...
          ErrorMessage.ShowLog("TemplateParser-log","show tables");
			...
			return Gen.Strings.Xml_DatabaseCollection.Replace(
				"{inner-content}",
				Gen.Strings.Xml_Database.Replace("{inner-content}",output)
			);
...
//					(ds.Tables["DATA_TYPES"] context, string filterCountry) =>
				...
				string newport = Gen.Strings.Xml_Table
			...
			{
				string newport = Gen.Strings.Tbl_TableElement
					.ReplaceFieldsT(ds,row,"TABLE_NAME","TABLE_SCHEMA","TABLE_TYPE")
		...
		static public string FilterColumns(this string input, DataSet ds, string table, bool ace)
		{
			...
			string fmt = "";
			if (string.IsNullOrEmpty(fmt)) fmt = Gen.Strings.Tbl_FieldElement;
...
//						}
		...
		{
			DataTable dataTypes = ds.Tables[Gen.Strings.Schema_DataTypes];
		...
		static public string FilterType(this DataRowView row, DataSet ds, string getField)
		{
			...
			foreach (DataRowView rv in ds.FindTable(Gen.Strings.Schema_DataTypes).DefaultView)
			{
				//CHARACTER_MAXIMUM_LENGTH(col)
				//DATA_TYPE(col)
